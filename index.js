...
app.get('/name', isAuthenticated,  function (req, res)  {
  console.log("Sending user name back");
  res.send({
...
// 
const profileDB = require("./sqlProfile.js");
let insertUser = `INSERT INTO Profile (userId, userName) values (?,?)`
let getUserById = "SELECT * FROM Profile WHERE userId = ?"
let selectAll = "SELECT * FROM Profile"
async function gotProfile(accessToken, refreshToken, profile, done) {
    ...
    let cmd = `SELECT * FROM Profile WHERE userId = ${id}`

    let result = await profileDB.all(cmd);
...
passport.deserializeUser((userid, done) => {
    ...
    let userData = {userData: "someString"};
    let result = async function (){
      return await profileDB.get(getUserById, [userid])
      .then((obj)=>{
